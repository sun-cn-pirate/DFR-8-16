...
ROC-AUC 比论文表中 12 层配置高约 `0.01013`，平均 PRO-AUC 低约
`0.01222`。逐类别结果和与论文的差异均如实保留，没有通过未披露调参修改结果。

## 与原论文结果对比

下表比较本次 700 epochs 复现与原论文 **12 层 VGG19 配置**的结果。
差值定义为“本次复现 − 原论文”；正数表示本次结果更高，负数表示本次结果更低。
所有数值均使用 `0–1` 标度。

| 类别 | 本次像素 ROC-AUC | 论文像素 ROC-AUC | 差值 | 本次 PRO-AUC | 论文 PRO-AUC | 差值 |
|---|---:|---:|---:|---:|---:|---:|
...
| **15 类平均** | **0.94746** | **0.93733** | **+0.01013** | **0.89045** | **0.90267** | **-0.01222** |

从逐类结果看，像素级 ROC-AUC 与论文总体一致并略高；PRO-AUC 的主要差异来自
`transistor`（低 `0.14237`），其余类别大多接近论文数值。原论文未在该对比表中
提供本仓库所计算的图像级 AP、图像级 ROC-AUC、像素级 AP 和 Best IoU，因此这些
扩展指标只报告本次实测值，不作不可靠的直接比较。
