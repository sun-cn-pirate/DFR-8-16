...
#         preds = scores.mean(1).mean(1)
        ...
        # Ground-truth regions do not change with the score threshold.  The
        # upstream implementation rebuilt connected-component labels and
        # region properties for every image at every threshold (up to 5000
        # times), which made the formal PRO evaluation needlessly expensive.
        # Cache the exact bounding boxes, filled masks, and areas once; this is
        # mathematically identical to the original loop below.
        regions_by_image = []
        for mask in masks:
            label_map = measure.label(mask, connectivity=2)
            regions_by_image.append([
                (prop.bbox, prop.image_filled, prop.area)
                for prop in measure.regionprops(label_map)
            ])
        anomalous_images = masks.any(axis=(1, 2))
        negative_masks = ~masks
        negative_pixel_count = negative_masks.sum()

        for step in range(max_step):
            thred = max_th - step * delta
            # segmentation
            np.greater(scores, thred, out=binary_score_maps)
            ...
            for i in range(len(binary_score_maps)):    # for i th image
                ...
                for bbox, cropped_mask, area in regions_by_image[i]:
                    x_min, y_min, x_max, y_max = bbox
                    cropped_pred_label = binary_score_maps[i][x_min:x_max, y_min:y_max]
                    intersection = np.logical_and(cropped_pred_label, cropped_mask).astype(np.float32).sum()
                    pro.append(intersection / area)

            # Compute per-image IoU in one vectorized pass, retaining the
            # upstream behavior of skipping images without anomalous pixels.
            intersections = np.logical_and(binary_score_maps, masks).astype(np.float32).sum(axis=(1, 2))
            unions = np.logical_or(binary_score_maps, masks).astype(np.float32).sum(axis=(1, 2))
            iou = intersections[anomalous_images] / unions[anomalous_images]
...
#             print("per image mean iou:", np.array(iou).mean())
            ...
            # fpr for pro-auc
            fpr = np.logical_and(negative_masks, binary_score_maps).sum() / negative_pixel_count
            fprs.append(fpr)
            threds.append(thred)
